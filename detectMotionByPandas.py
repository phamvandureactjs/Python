...
video = cv2.VideoCapture("./video/oven_animation_2.mp4")

# using infinite loop to capture the frames from the video
while True:
    # Reading each image or frame from the video using read function
    # check is bool variable to check if frame can be read, cur_frame is frames in video
    ...
    # convert image frame in to gray color
    gray_image = cv2.cvtColor(cur_frame, cv2.COLOR_BGR2GRAY)

    # To find the changes creating a GaussianBlur from the gray scale image
    # Using Gaussian filter to make frame more smooth and decrease noise
    ...
    for cur in cont:
        ...
        cv2.rectangle(
            cur_frame, (cur_x, cur_y), (cur_x + cur_w, cur_y + cur_h), (0, 255, 0), 3
        )
